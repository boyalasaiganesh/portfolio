...
const ContactInputMessage = styled.textarea`
  flex: 1;
  background-color: transparent;
  border: 1px solid black; /* Ensure the border color is black */
  outline: none;
  font-size: 18px;
  color: ${({ theme }) => theme.card_text};
  border-radius: 12px;
...
const ContactButton = styled.input`
  ...
  font-size: 18px;
  font-weight: 600;
  cursor: pointer; /* Ensure the cursor shows pointer */
`;
...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    emailjs.sendForm('service_zezgheu', 'template_g55i99i', form.current, 'bVzmYPrpc7f43XcU5')
      .then((result) => {
        setOpen(true);
